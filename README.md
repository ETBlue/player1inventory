...
<img src="docs/screenshots/Player%201%20Inventory%2011-pantry-filter.png" width="360" alt="Pantry with filters" />
...
Items with variable package sizes (e.g. milk in 1000ml or 2000ml cartons) can be tracked in measurement units instead of package counts.

<img src="docs/screenshots/feature-track-in-measurement.png" width="300" alt="Item tracked in measurement unit" />

Expiration is configurable per item: set a fixed due date, or let the app calculate it from the last purchase date.
...
Each item has a configurable refill threshold. The shopping page sorts items by stock level by default, so low-stock items rise to the top automatically. The list can be filtered by vendor so each household member sees only what to pick up at their store.

With cloud sync and family sharing enabled, anyone in the household can check what's running low on their way to the store — no real-time coordination needed.
...
Real-life household goods have complex state transitions, flexible categorization needs, and overlapping relationships that game inventory systems don't need to handle. These are the design challenges I encountered — and the decisions I made to address them.
...
**Stocking threshold** — users carry an implicit sense of "enough" in their heads. Making it explicit (target quantity + refill threshold) lets the shopping page sort low-stock items to the top automatically. Low-stock items also get a distinct color in the list view.
...
<img src="docs/screenshots/Player%201%20Inventory%2082-recipe-items.png" width="300" alt="Settings — recipe" />
