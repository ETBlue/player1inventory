...
One divergence remains: the cloud `Item` **still declares the five state columns** until PR 5 (they are dual-written server-side by `lib/stockDualWrite.ts` so a browser on a stale bundle keeps working, and every cloud read strips them before joining). Cloud **carts and inventory logs became location-scoped in PR 3a and PR 3b** — `Cart.locationId`, `InventoryLog.locationId`, and `Cart.id` re-keyed to `${locationId}:${vendorId | 'no-vendor'}` — so the `isCloud` bypasses shopping and cooking used to carry are gone too. v16 had already narrowed the configuration half — the cloud `Item` carried all eight configuration fields before this — so PR 2 closed what was left on the state half.
