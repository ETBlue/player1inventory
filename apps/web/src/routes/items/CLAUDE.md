...
- **Blur is where a field settles**, not keystroke: an empty/unparsable field resolves to `0`, and Unpacked's `roundToStep(…, consumeAmount || 1)` runs there too. Rounding on every keystroke destroyed a partially typed decimal — typing `2.5` landed on `3` before the `5` was pressed. The rounding itself is unchanged, only deferred.
