...
- **`consumeAmount === 0` means "no step size", honestly, everywhere in this form** (designer ruling, 2026-08-24). It is the create default since `6302ee97` — "not configured yet" — so nothing may fabricate a `1` for it. Both routes pass `item.consumeAmount ?? 0` into `initialValues` (the `??` is unreachable: `consumeAmount` is a required `number` on `Item`, the v16 upgrade backfills every pre-v16 row and `createItem` defaults it to `0`), and `ItemForm` derives one `quantityStep = consumeAmount > 0 ? consumeAmount : 'any'` for the Unpacked / Target Quantity / Refill When Below `step` attributes — `step={0}` is invalid HTML, so **"no step" is spelled `step="any"`**. Unpacked's blur normalizer passes `consumeAmount` straight to `roundToStep`, which returns the value untouched for a step `<= 0`; an unconfigured item's decimal is therefore kept, not silently rounded to a whole unit. The Info tab's `consumeAmount > 0` validation ("Must be greater than 0.") stays as the deliberate "needs setting up" signal. Pinned in `ItemForm.test.tsx` (display, the three `step="any"` attributes, no blur rounding, plus a `consumeAmount: 2` control that must still snap) and end-to-end in `$id.test.tsx`. Deliberately **not** applied to the stepper/recipe call sites, whose `|| 1` means something else there — a `+`/`−` increment must be non-zero to be usable, and a recipe `defaultAmount` of `0` means "optional, unchecked".
