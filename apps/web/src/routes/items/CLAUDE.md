...
**Cloud keeps the sequential path** (`persistInfo` then one `updateRecipe` per recipe): Apollo has no client-side transaction, so wrapping it would fake an atomicity that does not exist. Since PR 1 cloud *does* have per-location `ItemStock`, so "there is nothing to convert" is no longer the reason — the reason is that **the cloud schema has no `applyUnitSwitch` mutation**. The design names one as a requirement (§2) but PR 1 shipped `itemStock.graphql` without it, so `useApplyUnitSwitch` throws in cloud and `buildStockConversions` gates itself on `isLocal` rather than listing conversions this branch could not write. **Consequence, recorded rather than hidden: a cloud unit switch leaves every location's tracked quantities in the OLD unit.** PR 3 owes the mutation — **one combined GraphQL mutation** wrapping the item update, the per-location conversions and the recipe rewrites in a single server-side transaction.
