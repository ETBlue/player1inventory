...
- **The empty state is gated `!trimmedSearch && sortedItems.length === 0`.** These views render a flat sequence of sibling `&&` expressions inside one `<div className="flex flex-col gap-px">`, not a ternary chain, so PR B's short-circuit bug shape cannot occur here — the *additive* risk does: an ungated empty state would render "No items" **beside** the tail instead of yielding to it. **Known gap, all three detail views:** because the guard is `!trimmedSearch`, a group with no items plus a query matching nothing globally renders an empty list, an empty tail and no empty state — a blank pane. The fix needs no new derivation (`!hasTail && displayedItems.length === 0`, using the wiring hook's `hasTail`) but belongs to all three views at once; see `docs/features/items/2026-08-27-unified-item-search-plan-c.md`.
