...
**"Expires on" (designer ruling, 2026-08-27, reversing the 2026-05-31 "quick-update never touches `dueDate`" decision — see `docs/features/pantry/2026-05-31-brainstorming-quick-update-dialog.md`'s dated addendum).** A full-width date field renders as the LAST element, after the stepper grid, but only when `item.expirationMode === 'date'` — that global mode is Info-tab configuration, read here and never written; the due date itself is per-location `ItemStock` state, exactly like `ItemForm`'s Stock tab (which gates and formats it identically — see `components/item/ItemForm` above). `localDueDate` is seeded from `item.dueDate` (converted to the `YYYY-MM-DD` string `<input type="date">` needs, same conversion as `itemToFormValues` in `routes/items/$id/stock.tsx`) in the same reset effect as the four quantity locals, and `isUntouched` compares it alongside them so Update stays disabled until something actually changed.
...
**Stock settings rows.** Editable **Target** and **Refill below** steppers — the same per-location `ItemStock` fields the item form owns as "Target Quantity" / "Refill When Below", shortened here so the label column stays narrow. They **lead the grid**, as the first and second rows of the **same** `grid-cols-[auto_auto_auto]` grid as Packed/Unpacked — not a second grid of their own: two adjacent grids size their columns independently, so their steppers would not line up. In the merged four-row grid the third column carries the muted hints (`Inactive when 0`, `Warns on low stock`) on the first two rows and the Unpack/Pack buttons on the last two. Document order (progress bar → Target → Refill below → Packed → Unpacked) and the shared grid are both pinned by tests in `QuickUpdateDialog.test.tsx`. Both settings are labelled and stored in the item's **tracking** unit (the unpacked row's unit), not the package unit. Target steps by 1 in `package` mode and by `consumeAmount` otherwise (mirroring `ItemForm`'s `step={targetUnit === 'package' ? 1 : quantityStep}`); Refill below always steps by `consumeAmount`. Both clamp at 0.
