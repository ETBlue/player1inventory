...
- **Words = which location is globally active.** The dots do not mark it at all (fill is spent on page position), so the caption under the heading carries it on every page — "Active" when standing on it, "Active: `<name>`" while viewing another page. That caption is the **only** sighted cue for the active location. The active dot also keeps "(active location)" in its accessible name, and the viewed tab keeps `aria-selected`.
