...
**`QuickUpdateDialog`** (`src/components/item/QuickUpdateDialog/index.ts`) — pantry-page dialog for bulk-editing a single item's stock in one submit. Triggered by the calculator icon button on each `ItemCard` in pantry mode. Provides +/− steppers, manual inputs, Pack/Unpack buttons (mirrors item info tab logic via `computePack`/`computeUnpack`), Clear, and Fill to Full actions, with a live progress bar preview. Submits a single mutation with the final `{ packedQuantity, unpackedQuantity, targetQuantity, refillThreshold }` — never touches `dueDate`. Pack/Unpack/Fill-to-Full logic lives in pure functions in `quantityUtils.ts` (`computePack`, `computeUnpack`, `computeFillToFull`).

**Stock settings rows.** Editable **Target** and **Refill below** steppers — the same per-location `ItemStock` fields the item form owns as "Target Quantity" / "Refill When Below", shortened here so the label column stays narrow. They sit **directly above the progress bar**, as the third and fourth rows of the **same** `grid-cols-[auto_auto_auto]` grid as Packed/Unpacked — not a second grid below it: two adjacent grids size their columns independently, so their steppers would not line up. In the merged four-row grid the third column carries the Unpack/Pack buttons on the first two rows and the muted hints (`Inactive when 0`, `Warns on low stock`) on the last two. Document order (Packed → Unpacked → Target → Refill below → progress bar) and the shared grid are both pinned by tests in `QuickUpdateDialog.test.tsx`. Both settings are labelled and stored in the item's **tracking** unit (the unpacked row's unit), not the package unit. Target steps by 1 in `package` mode and by `consumeAmount` otherwise (mirroring `ItemForm`'s `step={targetUnit === 'package' ? 1 : quantityStep}`); Refill below always steps by `consumeAmount`. Both clamp at 0.
