...
**Stock settings row.** Below the progress bar sit editable **Target** and **Refill ≤** steppers — the same per-location `ItemStock` fields the item form owns as "Target Quantity" / "Refill When Below", shortened here so the label column stays narrow. Both are labelled and stored in the item's **tracking** unit (the unpacked row's unit), not the package unit. Target steps by 1 in `package` mode and by `consumeAmount` otherwise (mirroring `ItemForm`'s `step={targetUnit === 'package' ? 1 : quantityStep}`); Refill ≤ always steps by `consumeAmount`. Both clamp at 0.
