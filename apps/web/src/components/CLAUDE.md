...
**`NewItemDialog`** (`src/components/item/NewItemDialog/index.ts`) — the pantry **"Add" dialog**, a searchable **combobox** over all items the user can access (every global `Item`, via `useItems()`). Props: `open`, `onOpenChange`, `initialName?` (pre-fills + focuses the combobox), `onSuccess?(item)`, `catalogOnly?`. Behaviour:
...
- **`catalogOnly` (default `false`) makes the create path global.** With it, `useCreateItem({ catalogOnly: true })` writes the `Item` but **no `ItemStock`** — the new item lands in the catalog stocked in no location (an intentional orphan). Opt-in rather than the default because the pantry's Add flow shares this dialog and must keep stocking here; the tag/vendor/recipe Settings assignment tabs pass it because they edit a *global* item↔entity relation. It governs the **create path only** — select-existing still stocks the chosen item in the active location via `useAddItemToLocation()`, a known asymmetry recorded in `routes/settings/CLAUDE.md`.

Used by: pantry views (Add button) and the tag/vendor/recipe items tabs (which pass `onSuccess` to assign the new/added item, plus `catalogOnly`). Tests that submit the create path must click the dialog's `Create "<name>"` button (`within(dialog).getByRole('button', { name: /create/i })`), not the old "New Item" button.
