...
Two independent visual channels so they compose: **fill** marks the page being viewed (`bg-importance-primary-background` vs `bg-accessory-emphasized`), **ring** marks the globally active location (`ring-importance-primary-accessory` + `ring-offset-background-elevated`), which stays put while the user pages elsewhere. Neither marker is colour-only — the viewed page is `aria-selected` and named in the heading above the dots; the active location carries "(active location)" in its accessible name plus a visible "Active" hint. The caller decides whether to render it at all: with a single location the Stock tab renders no chrome.
