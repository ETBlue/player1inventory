...
const stockMilk = (vendorIds: string[]) =>
  createItem(
    {
      name: 'Milk',
      tagIds: [],
...
describe('VendorDetailView quick update', () => {
  beforeEach(async () => {
  ...
  it('user can edit target and refill from a vendor page and have all four stock fields saved', async () => {
    // Given a vendor selling an item with target 6 / refill 2
    const vendor = await createVendor('Corner Store')
    const milk = await stockMilk([vendor.id])
    const user = userEvent.setup()
    await renderWithRouter(<VendorDetailView vendorId={vendor.id} />)
  ...
  it('user can edit the due date from a vendor page when the item is in date mode', async () => {
    // Given a vendor selling an item in date mode with a stored due date
    const vendor = await createVendor('Corner Store')
