...
- `useScrollRestoration(key)` (`src/hooks/useScrollRestoration.ts`) - Saves/restores the scroll position for a given key (typically the full URL). Targets `main#main-content` as the scroll container (falls back to `document.documentElement`). Saves `scrollTop` on component unmount (SPA navigation away); restores after data loads. Usage: call with `currentUrl` from `useRouterState`, then call `restoreScroll()` in a `useEffect` conditioned on `!isLoading`.
