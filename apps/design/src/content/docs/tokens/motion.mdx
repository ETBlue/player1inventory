...
Each appears once, inside shadcn/ui base components (`dialog.tsx` close button and `switch.tsx` thumb). These are not patterns to reach for in feature code — use `transition-colors` or `transition-all` instead.
...
Appears in: `LoadingSpinner`, `button.tsx` (loading variant), `DeleteButton`, and several route-level loading states.
