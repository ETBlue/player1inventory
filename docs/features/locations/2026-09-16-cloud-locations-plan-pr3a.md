...
| 4 | **"useItems.ts:1049 — the count query, cloud must match" names a cloud caller that does not exist.** `useInventoryLogCountByItem` is Dexie-only. The generated `useInventoryLogCountByItemQuery` is referenced by two test stubs and by no component; the cloud `InventoryLogCountByItem` operation document has no caller at all. The Stock tab's remove confirmation prints those counts **in local mode only**, which `routes/items/CLAUDE.md` already records. The document still gained `$locationId: ID!` so the schema and the operations stay in step. |
| 4 | The plan listed `useItemSortData.ts` and `useShoppingCart.ts` as the places calling `lastPurchaseDates`. `useShoppingCart.ts` does not call it — it only **evicts** the `ROOT_QUERY.lastPurchaseDates` field after checkout, and that eviction names no arguments so it still clears every location's entry. The second caller is `useLastPurchaseDate` in `useItems.ts`. |
| 4 | **Making `locationId` required needs a read-side gate, which the plan did not mention.** With the fallback gone, a cloud read sent during the fresh-session window carries the `'local'` sentinel and comes back `FORBIDDEN`. PR 2 had already solved this for `PantryData` with a private `useCloudLocationKnown` inside `useItems.ts`; Task 4 moved it to `src/hooks/useCloudLocationKnown.ts` so the three inventory-log readers can use it too. |
| 4 | **Step 4.4 found nothing to fix.** `useAddInventoryLog`'s cloud branch has no `refetchQueries` at all, by name or otherwise, so there is no observer for a by-name refetch to reach. Nothing was added: an `ItemLogs` refetch by name is exactly the hazard the step warns about. |
| 4 | The plan's mutation check 4 says a cache-key test "switches location and reads stale logs". Measured: a hook-level switch test stays **GREEN** when `'locationId'` is dropped from the `itemLogs` `keyArgs`, because `useItemLogs` reads `cache-and-network` and refetches on every switch. What a collapsed key costs is the wrong list rendered while that refetch is in flight. The real guard is `apollo/client.test.ts`, which reads the cache directly. |
