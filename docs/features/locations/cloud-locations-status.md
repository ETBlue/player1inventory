...
| **2** | ✅ | The web client's cloud path moves onto `Location` / `ItemStock`. Writes split client-side. A four-site server dual-write keeps `Item`'s legacy columns fed until PR 5. |
...
A **four-site server dual-write** keeps `Item`'s legacy columns fed until PR 5. The fourth
site is `upsertItemStock`'s reverse mirror. It was added because the PR-2 client stopped
sending stock to `updateItem` at all.
...
### Rehearsal 2 — still required, before PR 3

It re-measures the `'no-vendor'` count against whatever production looks like then. Running
it fresh is the point; the Rehearsal 1 number must not be trusted in its place.
