# TemplateItemsBrowser Toolbar Alignment Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Align `TemplateItemsBrowser` header with the pantry page toolbar — same background/border, Tags/Filters/Search toggle buttons starting closed, and same borderless search input.

**Architecture:** Single-file change to `TemplateItemsBrowser/index.tsx`. Replace the hand-rolled header div with the shared `<Toolbar>` component, restructure Row 1 to add three toggle buttons, and make Rows 2–4 conditional on the new boolean state vars. Update Storybook stories and smoke test.

**Tech Stack:** React 19, TypeScript strict, Tailwind CSS v4, react-i18next, Vitest + React Testing Library, Storybook

---

### Task 1: Update `TemplateItemsBrowser` — toolbar restructure
...
Replace the full content of `apps/web/src/components/onboarding/TemplateItemsBrowser/index.tsx` with:
...
Replace the full content of `apps/web/src/components/onboarding/TemplateItemsBrowser/TemplateItemsBrowser.stories.tsx` with:
...
(Stories are unchanged — the component starts with all rows closed, so the default stories still show a clean toolbar.)

- [ ] **Step 3: Update smoke test to assert the new toggle buttons**

Replace the full content of `apps/web/src/components/onboarding/TemplateItemsBrowser/TemplateItemsBrowser.stories.test.tsx` with:
...
```bash
(cd apps/web && pnpm test --run TemplateItemsBrowser)
```
...
git commit -m "feat(onboarding): align TemplateItemsBrowser toolbar with pantry page visual"
```
