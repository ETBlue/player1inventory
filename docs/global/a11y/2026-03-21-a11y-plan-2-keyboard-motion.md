...
**Issue:** Tag reordering on the settings/tags page uses `@dnd-kit` with only `PointerSensor` and `TouchSensor`. Keyboard-only users cannot reorder tags. The dnd-kit library includes a `KeyboardSensor` that enables arrow-key navigation for drag operations.
...
Read `src/routes/settings/tags/index.tsx`. Locate the `DndContext` setup and sensor configuration. Find where `PointerSensor` and `TouchSensor` are imported and used.
...
Add a visually-hidden instruction string near the draggable list so screen reader users know they can use arrow keys:
...
Add `aria-describedby="dnd-instructions"` to each draggable item's handle or root element.
