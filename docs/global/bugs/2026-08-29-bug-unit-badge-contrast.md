...
- **No number spacing changed** — `GroupCard` still emits `12/20`,
  `getStockPreview` still emits `3 / 4`. Only the unit was appended.
...
match no longer sees `0/0 jug` — so it is a regex now too. Mutating
`showStock &&` to `true &&` in `ItemCard` turns both red.
